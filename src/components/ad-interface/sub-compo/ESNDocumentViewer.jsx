...
// ESNDocumentViewer component
const ESNDocumentViewer = ({ esnId }) => {
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      try {
        ...
        if (response) {
          // Transform API data to our document structure based on Doc_ESN model
        ...
        message.error("Impossible de charger les documents de l'ESN");
        setDocuments([]);
      } finally {
        setLoading(false);
      }
  ...
  const viewDocument = (doc) => {
    ...
    if (doc.extension === "pdf") {
      ...
      // For non-PDF files, use the modal viewer
  ...
  const downloadDocument = (doc) => {
    try {
      ...
      link.href = doc.url; // This already contains ${Endponit()}/media/${doc.Doc_URL}
  ...
  if (documents.length === 0) {
    return (
      <Empty
        description="Aucun document disponible pour cette ESN"
...
export default ESNDocumentViewer;
